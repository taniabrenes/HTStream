...
argCollector::argCollector(int argc, char *argv[], non **n) {
    ...
    sprintf(tmplog, "non.log");

    while ((cmd_line_char = getopt_long(argc, argv, "m:VNL:1:2:U:I:T:OgiftP:FSh", longopts, &long_index)) != EOF) {
        switch(cmd_line_char) {
            case 'm':
                (*n)->setLength(atoi(optarg));
                break;
            case 'N':
                log = stderr;
                break;
    ...
    if (!prefix) {
        prefix = (char *)malloc(4*sizeof(char));
        sprintf(prefix, "non");
    }
    ...
    if (tab) {
        ...
        if (R1_In || SE_In || TAB_In || STDIN_In) {
